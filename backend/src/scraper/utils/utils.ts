...
export function inferSex(
  ...
  if (womenKeywords.some((k) => lowerTags.some((tag) => tag.includes(k))))
    return 'women';
  if (menKeywords.some((k) => lowerTags.some((tag) => tag.includes(k))))
    return 'men';
...
export function normalizeCategory(raw: string): string {
  ...
  // Pants & Sweatpants
  if (/\b(pants|trousers|chinos|sweatpants|joggers|trackpants)\b/.test(s)) {
  ...
  // Jumpers & Cardigans
  if (/\b(jumpers?|hoodies?|sweatshirts?|sweaters?|cardigans?)\b/.test(s)) {
