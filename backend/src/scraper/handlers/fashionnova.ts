...
export async function extractProductUrls(src: string): Promise<string[]> {
  ...
  if (!body.startsWith('<')) {
    ...
    const urls = Array.from(
      new Set(
        parts.filter(
          (tok) => tok.startsWith('http') && tok.includes('/products/'),
        ),
      ),
    );
  ...
  const urls = Array.from(
    new Set(
      entries
        ...
        .filter(
          (u: any): u is string =>
            typeof u === 'string' && u.includes('/products/'),
        ),
    ),
  );
...
export async function buildUpsert(
  ...
  if (!p) return null;
  // skip accessories
  if (/(jewelry|sunglasses|accessories|bags|hats)/i.test(p.productType ?? ''))
    return null;
  ...
  const listPrice = parseFloat(
    p.selectedVariant?.compareAtPrice?.amount ?? price.toString(),
  );
  ...
  return {
    ...
    create: {
      ...
      productImages: {
        createMany: { data: images.map((i) => ({ imageUrl: i })) },
      },
    },
  };
}
