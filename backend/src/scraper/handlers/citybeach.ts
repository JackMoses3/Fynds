...
export async function extractProductUrls(src: string): Promise<string[]> {
  ...
  return urls.filter(
    (u) => typeof u === 'string' && /\/\d+-\d+\.html$/.test(u),
  );
}
...
export async function buildUpsert(
  ...
  let p: any;
  try {
    const resp = await client.get<{ product: any }>(variationUrl, {
  ...
  // skip non-clothing
  if (
    !Array.isArray(p.productTypelvl1) ||
    !p.productTypelvl1.includes('Clothing')
  ) {
    console.log(`   • skipping non-clothing ${pageUrl}`);
    return null;
  }

  const categoryRaw = Array.isArray(p.productTypelvl3)
    ? p.productTypelvl3[0]
    : '';
  const category = normalizeCategory(categoryRaw);

  const price =
    parseFloat(p.customProps?.currentPrice) ||
    parseFloat(p.price?.sales?.value) ||
    0;
  ...
  return {
    ...
    create: {
      ...
      itemVideos: {
        createMany: { data: videos.map((u) => ({ videoUrl: u })) },
      },
    },
  };
}
