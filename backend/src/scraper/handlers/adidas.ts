...
async function extractProductUrls(fromUrl: string): Promise<string[]> {
    ...
    const entries = (parsed.urlset?.url as Array<{ loc: string[] }> | undefined) ?? [];
    const urls = entries
        .map(entry => entry.loc[0])
        .filter((loc): loc is string => typeof loc === 'string' && loc.endsWith('.html'));
...
export async function handleAdidas(
    ...
    for (const raw of config.siteMapUrl) {
        ...
        for (const pageUrl of productUrls) {
            ...
            const standardPrice: number = data.pricing_information?.standard_price ?? currentPrice;
