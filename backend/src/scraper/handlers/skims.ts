...
export async function extractProductUrls(src: string): Promise<string[]> {
  let xml: string;
  if (src.endsWith('.gz')) {
    const resp = await client.get<ArrayBuffer>(src, {
...
export async function buildUpsert(
  ...
  try {
    let raw = await client
      .get<string>(apiUrl, { responseType: 'text', headers: JSON_HEADERS })
      .then((r) => r.data);
    if (raw.startsWith(")]}'")) {
  ...
  const p = dataObj.productGalleryData?.activeProduct;
  if (!p) {
  ...
  const tagsArray: string[] = Array.isArray(dataObj.productTagsDowncaseArray)
    ? dataObj.productTagsDowncaseArray
    : [];
  const genderTag = tagsArray.find((t) => t.startsWith('filtergender:'));
  const sex: string = genderTag
    ? genderTag.split(':')[1].trim()
    : inferSex('', []);
  ...
  // media: images & videos
  const pmImages = Array.isArray(dataObj.productMedia?.images)
    ? dataObj.productMedia.images
    : [];
  const images = pmImages.flatMap((m: any) =>
    m.image?.url ? [m.image.url] : [],
  );
  const videos = pmImages.flatMap((m: any) =>
    Array.isArray(m.videos) ? m.videos.map((v: any) => v.url) : [],
  );
  ...
  return {
    ...
    create: {
      ...
      productImages: {
        createMany: { data: images.map((u: string) => ({ imageUrl: u })) },
      },
      itemVideos: {
        createMany: { data: videos.map((u: string) => ({ videoUrl: u })) },
      },
    },
  };
}
