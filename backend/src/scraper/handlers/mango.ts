...
export async function buildUpsert(
  ...
  try {
    meta = (await client.get(metaUrl)).data;
  } catch (err) {
    console.error(`   • metadata fetch failed ${metaUrl}:`, err);
    return null;
  }

  try {
    priceData = (await client.get(priceUrl)).data;
  } catch (err) {
    console.error(`   • price fetch failed ${priceUrl}:`, err);
    return null;
  }
  ...
  const priceEntries: any[] = Object.values(priceData).map(
    (entry) => (entry as any).price,
  );
...
export async function handleMango(
  ...
  const sitemaps = config.siteMapUrl.flatMap((s) =>
    s
      .split(',')
      .map((x) => x.trim())
      .filter(Boolean),
