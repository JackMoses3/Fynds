...
export async function extractProductUrls(src: string): Promise<string[]> {
    ...
    const urls = entries
        .map((e: any) => e.loc)
        .filter((u: any): u is string => typeof u === 'string' && u.includes('/product/'));
...
export async function buildUpsert(pageUrl: string, config: SiteDataConfig,): Promise<UpsertPayload | null> {
    ...
    const images: string[] = Array.isArray(p.images) ? p.images.map((i: any) => i.url).filter((u: any): u is string => !!u) : [];
    ...
    const baseFields = { name, metaData, brand: 'Ghanda', price, standardPrice: listPrice, sale, retailer: 'Ghanda', category, sex };
