...
/* ------------------------------------------------------------------ */
const EXCLUDED = [
    'accessories', 'shoes', 'hats', 'apartment', 'home', 'skin',
    'skincare', 'beauty', 'makeup', 'hair', 'lifestyle', 'music',
    'intimates', 'cross',
];
...
async function buildUpsert(pageUrl: string): Promise<UpsertPayload[] | null> {
    ...
    const rawCat = Array.isArray(data.categories) && data.categories.length
        ? data.categories[0]
        : '';
    ...
    for (const colourOpt of colorVariants) {
        ...
        const allList = colourOpt.options?.map((sz: any) => sz.price?.list?.total ?? 0) || [];
        const allSale = colourOpt.options?.map((sz: any) => sz.price?.sale?.total ?? 0) || [];
