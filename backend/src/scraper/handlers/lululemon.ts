...
export async function extractProductUrls(src: string): Promise<string[]> {
    ...
    return Array.from(
        new Set(
            entries
                .map((e: any) => e.loc)
                .filter((u: any): u is string => typeof u === 'string' && /\/p\//.test(u))
        )
    );
}
...
export async function buildUpsert(pageUrl: string, config: SiteDataConfig,): Promise<UpsertPayload | null> {
    ...
    const pid = match[1];
    const apiUrl =
        `https://www.lululemon.com.au/on/demandware.store/Sites-AU-Site/en_AU/Product-Variation?dwvar_${pid}&pid=${pid}&quantity=1`;
    ...
    const category: string = normalizeCategory(cat.primaryCategory || cat.unifiedID || '');
    const sex: string = inferSex(cat.unifiedID || '', []);
    const images: string[] = (resp.images?.['hi-res'] ?? []).map((i: any) => i.url).filter(Boolean);

    const base = { name, brand, price, sale, retailer: 'Lululemon', category, sex, metaData: '' };
...
export async function handleLululemon(
    ...
    const sources = config.siteMapUrl.flatMap(s => s.split(',').map(x => x.trim()).filter(Boolean));
