...
export class ScraperService {
    ...
    private static websiteConfigs: { [key: string]: ScraperConfig } = {
        ...
        "platypusshoes.com.au": {
            retailer: "Platypus Shoes",
            productLinksSelector: 'div[data-analytics-plp="prod-results"] div.sc-eifFE.iuDnhZ.productCard a.sc-jokEwN',
            loadMoreSelector: '.product-grid-load-more button',
            imageScraper: async (page) => {
                return await page.evaluate(() => {
                    return Array.from(
                        document.querySelectorAll('div.product-images-grid img.product-detail-image')
                    ).map(img => {
                        const image = img as HTMLImageElement; // <--- Type casting here
